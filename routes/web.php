...
Route::middleware(['auth', 'role:student,instructor,admin'])
    ...
    ->group(function () {
        ...
        Route::get('/', [StudentDashboard::class, 'index'])
            ->name('index');

        Route::get('/my-courses', [StudentDashboard::class, 'myCourses'])
            ->name('courses');

        Route::get('/certificates', [StudentDashboard::class, 'certificates'])
            ->name('certificates');

        Route::get('/wishlist', [WishlistController::class, 'index'])
            ->name('wishlist');

        Route::get('/learn/{slug}', [LearningController::class, 'index'])
            ->name('learn');

        Route::get('/learn/{slug}/{lesson}', [LearningController::class, 'lesson'])
            ->name('learn.lesson');

        Route::post('/progress/{lesson}', [LearningController::class, 'updateProgress'])
            ->name('learn.progress');

        Route::get('/profile', [StudentDashboard::class, 'profile'])
            ->name('profile');

        Route::post('/profile/update', [StudentDashboard::class, 'updateProfile'])
            ->name('profile.update');

        Route::post('/profile/password', [StudentDashboard::class, 'updatePassword'])
