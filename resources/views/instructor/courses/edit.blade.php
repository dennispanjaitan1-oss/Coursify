...
@section('content')
    ...
    <div style="margin-bottom: 32px;">
        <h1 style="font-family: var(--font-serif); font-size: 36px; font-weight: 400; color: var(--text);">
            Edit <em>course</em> details
        </h1>
        <p style="color: var(--muted); font-size: 14px; margin-top: 4px;">Update details for: <strong>{{ $course->title }}</strong></p>
    </div>
    ...
    <form action="{{ route('instructor.courses.update', $course->id) }}" method="POST" style="display: grid; grid-template-columns: 1.8fr 1fr; gap: 28px; align-items: start; margin-bottom: 48px;">
        @csrf
        @method('PUT')
        ...
        <div style="display: flex; flex-direction: column; gap: 24px;">
            ...
            <div class="card-wrap" style="display: flex; flex-direction: column; gap: 20px;">
                ...
                <div style="display: flex; flex-direction: column; gap: 6px;">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $course->title) }}" required placeholder="e.g. Mastering Advanced Web Development" style="width: 100%; padding: 11px 16px; border: 1.5px solid var(--border); border-radius: var(--radius-sm); font-size: 13.5px; font-family: var(--font-sans); transition: var(--transition-fast); outline: none;" onfocus="this.style.borderColor='var(--purple)'" onblur="this.style.borderColor='var(--border)'">
                </div>

                <div style="display: flex; flex-direction: column; gap: 6px;">
                    <label for="slug" style="font-weight: 600; font-size: 13px; color: var(--text-soft);">URL Slug <span style="color: var(--orange);">*</span></label>
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $course->slug) }}" required placeholder="e.g. mastering-advanced-web-development" style="width: 100%; padding: 11px 16px; border: 1.5px solid var(--border); border-radius: var(--radius-sm); font-size: 13.5px; font-family: var(--font-sans); transition: var(--transition-fast); outline: none; background: #FAF9FD;" onfocus="this.style.borderColor='var(--purple)'; this.style.background='white';" onblur="this.style.borderColor='var(--border)'; this.style.background='#FAF9FD';">
            ...
            <div class="card-wrap" style="display: flex; flex-direction: column; gap: 20px;">
                ...
                <div style="display: flex; flex-direction: column; gap: 6px;">
                    ...
                    <input type="url" name="thumbnail_url" id="thumbnail_url" value="{{ old('thumbnail_url', $course->thumbnail_url) }}" placeholder="https://images.unsplash.com/... or your custom asset image URL" style="width: 100%; padding: 11px 16px; border: 1.5px solid var(--border); border-radius: var(--radius-sm); font-size: 13.5px; font-family: var(--font-sans); transition: var(--transition-fast); outline: none;" onfocus="this.style.borderColor='var(--purple)'" onblur="this.style.borderColor='var(--border)'">
                </div>

                <div style="display: flex; flex-direction: column; gap: 6px;">
                    <label for="preview_video_url" style="font-weight: 600; font-size: 13px; color: var(--text-soft);">Course Intro Video URL</label>
                    <input type="url" name="preview_video_url" id="preview_video_url" value="{{ old('preview_video_url', $course->preview_video_url) }}" placeholder="https://www.youtube.com/watch?v=... or direct mp4 url" style="width: 100%; padding: 11px 16px; border: 1.5px solid var(--border); border-radius: var(--radius-sm); font-size: 13.5px; font-family: var(--font-sans); transition: var(--transition-fast); outline: none;" onfocus="this.style.borderColor='var(--purple)'" onblur="this.style.borderColor='var(--border)'">
                </div>
            </div>
        </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 24px; position: sticky; top: 24px;">
            ...
            <div class="card-wrap" style="display: flex; flex-direction: column; gap: 20px;">
                ...
                <div style="display: flex; flex-direction: column; gap: 6px;">
                    ...
                    <input type="number" name="price" id="price" value="{{ old('price', $course->price) }}" min="0" required placeholder="0 for free" style="width: 100%; padding: 11px 16px; border: 1.5px solid var(--border); border-radius: var(--radius-sm); font-size: 13.5px; font-family: var(--font-sans); transition: var(--transition-fast); outline: none;" onfocus="this.style.borderColor='var(--purple)'" onblur="this.style.borderColor='var(--border)'">
                </div>
                ...
                <div style="display: flex; flex-direction: column; gap: 6px;">
                    ...
                    <input type="number" name="duration_weeks" id="duration_weeks" value="{{ old('duration_weeks', $course->duration_weeks) }}" min="1" max="104" placeholder="e.g. 8" style="width: 100%; padding: 11px 16px; border: 1.5px solid var(--border); border-radius: var(--radius-sm); font-size: 13.5px; font-family: var(--font-sans); transition: var(--transition-fast); outline: none;" onfocus="this.style.borderColor='var(--purple)'" onblur="this.style.borderColor='var(--border)'">
                </div>

                <div style="display: flex; flex-direction: column; gap: 6px;">
                    <label for="language" style="font-weight: 600; font-size: 13px; color: var(--text-soft);">Language</label>
                    <input type="text" name="language" id="language" value="{{ old('language', $course->language ?? 'ID') }}" placeholder="ID or EN" style="width: 100%; padding: 11px 16px; border: 1.5px solid var(--border); border-radius: var(--radius-sm); font-size: 13.5px; font-family: var(--font-sans); transition: var(--transition-fast); outline: none;" onfocus="this.style.borderColor='var(--purple)'" onblur="this.style.borderColor='var(--border)'">
                </div>

                <div style="display: align-items: center; justify-content: space-between; border-top: 1.5px solid var(--border); padding-top: 18px; margin-top: 8px; display: flex;">
                    <div style="display: flex; flex-direction: column;">
                        <span style="font-weight: 700; font-size: 13.5px; color: var(--text);">Published Status</span>
                        <span style="font-size: 11px; color: var(--muted);">Visible to all prospective students</span>
                    </div>
                    <label class="switch" style="position: relative; display: inline-block; width: 44px; height: 24px;">
                        <input type="checkbox" name="is_published" value="1" {{ old('is_published', $course->is_published) ? 'checked' : '' }} style="opacity: 0; width: 0; height: 0;">
